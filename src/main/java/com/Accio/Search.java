...
public class Search extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        // Getting keyword from frontend//niche
        String keyword = request.getParameter("keyword");
        // Getting up connection to database
        Connection connection = DatabaseConnection.getConnection();
        try {
            // Store the History query
            PreparedStatement preparedStatement = connection.prepareStatement("Insert into history values(?,?);");
            preparedStatement.setString(1,keyword);
            //
            preparedStatement.setString(2,"http://localhost:8080/SearchEngine_2/Search?keyword="+keyword);
            preparedStatement.executeUpdate();

            // Getting result after running getting query
            ...
            // Displaying results in arraylist console
            for (SearchResult result:results) {
                System.out.println(result.getTitle()+"\n"+result.getLink()+"\n");
            }
            request.setAttribute("results",results);//this
